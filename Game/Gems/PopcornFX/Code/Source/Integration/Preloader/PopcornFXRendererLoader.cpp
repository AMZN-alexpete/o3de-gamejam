...
bool	PopcornFXRendererLoader::AddMaterialToCreate(const PAtomRendererCache &rendererCache)
{
	...
	if (!materialCacheExist)
	{
		...
		if (materialCacheKey.m_DiffuseMapPath.Valid())
		{
			...
			{
                AZ_Printf("PopcornFX", "Could not load diffuse texture from path '%s'", materialCacheKey.m_DiffuseMapPath.ToStringData());
				return false;
			}
		}
		if (materialCacheKey.m_DiffuseRampMapPath.Valid())
		{
			...
			{
                AZ_Printf("PopcornFX", "Could not load diffuse ramp texture from path '%s'", materialCacheKey.m_DiffuseRampMapPath.ToStringData());
				return false;
			}
		}
		if (materialCacheKey.m_EmissiveMapPath.Valid())
		{
			...
			{
                AZ_Printf("PopcornFX", "Could not load emissive texture from path '%s'", materialCacheKey.m_EmissiveMapPath.ToStringData());
				return false;
			}
		}
		if (materialCacheKey.m_EmissiveRampMapPath.Valid())
		{
			...
			{
                AZ_Printf("PopcornFX", "Could not load emissive ramp from path '%s'", materialCacheKey.m_EmissiveRampMapPath.ToStringData());
				return false;
			}
		}
		if (materialCacheKey.m_NormalMapPath.Valid())
		{
			...
			{
                AZ_Printf("PopcornFX", "Could not load normal map from path '%s'", materialCacheKey.m_NormalMapPath.ToStringData());
				return false;
			}
		}
		if (materialCacheKey.m_MotionVectorsMapPath.Valid())
		{
			...
			{
                AZ_Printf("PopcornFX", "Could not load motion vectors map from path '%s'", materialCacheKey.m_MotionVectorsMapPath.ToStringData());
				return false;
			}
		}
		if (materialCacheKey.m_AlphaMapPath.Valid())
		{
			...
			{
                AZ_Printf("PopcornFX", "Could not load motion alpha map from path '%s'", materialCacheKey.m_AlphaMapPath.ToStringData());
				return false;
			}
		}
		if (materialCacheKey.m_DistortionMapPath.Valid())
		{
			...
			{
                AZ_Printf("PopcornFX", "Could not load distortion map from path '%s'", materialCacheKey.m_DistortionMapPath.ToStringData());
				return false;
			}
		}
	}
	...
	if (!pipelineStateCacheExist)
	{
        ...
        {
            AZ_Printf("PopcornFX", "Could not load shader from path '%s'", shaderPathPtr);
            return false;
        }
		...
		if (IsLitShader(pipelineCacheKey.m_UsedShader))
		{
			if (pipelineCacheKey.m_BlendMode == BlendMode::Solid || pipelineCacheKey.m_BlendMode == BlendMode::Masked)
			{
                ...
                {
                    AZ_Printf("PopcornFX", "Could not load shader from path '%s'", opaqueDepthPass);
                    return false;
                }
			}
			else
			{
                ...
                {
                    AZ_Printf("PopcornFX", "Could not load shader from path '%s'", minDepthPass);
                    return false;
                }
                ...
                {
                    AZ_Printf("PopcornFX", "Could not load shader from path '%s'", maxDepthPass);
                    return false;
                }
			}
		}
    ...
    {
        AZ_Printf("PopcornFX", "Could not link and update material render cache");
        return false;
    }

	if (!_LinkAndUpdateRendererCacheIFP(rendererCache, pipelineStateCache, CAtomRendererCache::CacheType_PipelineState))
    {
        AZ_Printf("PopcornFX", "Could not link and update pipeline state render cache");
        return false;
    }
...
bool	PopcornFXRendererLoader::AddGeometryToCreate(const PAtomRendererCache &rendererCache)
{
    ...
    {
        AZ_Printf("PopcornFX", "Cannot create geometry because mesh path is empty");
        return false;
    }
    ...
    {
        AZ_Printf("PopcornFX", "Failed to create geometry cache with path '%s'", path.Data());
        return false;
    }
	...
	if (!geometryCacheExist)
	{
        ...
        {
            AZ_Printf("PopcornFX", "Failed to add geometry to load when creating geometry cach from path '%s'", path.Data());
            return false;
        }
	}
    ...
    {
        AZ_Printf("PopcornFX", "Failed to link and update renderer cache when creating geometry cache from path '%s'", path.Data());
        return false;
    }
